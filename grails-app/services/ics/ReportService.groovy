...
class ReportService {
	...
	def query = "select * from (SELECT '' ccname,d.fund_receipt_date date,concat(d.nvcc_receipt_book_no,'/',d.nvcc_receipt_no) ref, concat(ifnull(d.collection_type,''),' by ',IFNULL(i.initiated_name,i.legal_name)) details, d.donor_contact contact, d.donor_email email, d.amount income, 0 expense, 0 balance, d.updator entryby FROM donation d, scheme s, individual i where d.donated_by_id=i.id and d.scheme_id=s.id and s.cc_id="+cc.id+" and d.fund_receipt_date >= '"+String.format('%tF',fd)+"'  and d.fund_receipt_date <= '"+String.format('%tF',td)+"' union SELECT c.name ccname,v.voucher_date date,v.voucher_no ref, v.description details, '','',v.amount_settled income, v.amount expense, 0 balance, v.updator entryby FROM voucher v, cost_center c where v.department_code_id=c.id and c.id="+cc.id+" and v.voucher_date >= '"+String.format('%tF',fd)+"' and v.voucher_date <= '"+String.format('%tF',td)+"') q  order by date"
	//log.debug(query)
	...
	def sql = new Sql(dataSource)
	def query = "select * from (SELECT cc.name ccname,d.fund_receipt_date date,concat(d.nvcc_receipt_book_no,'/',d.nvcc_receipt_no) ref, concat(ifnull(d.collection_type,''),' by ',IFNULL(i.initiated_name,i.legal_name)) details, d.donor_contact contact, d.donor_email email,d.amount income, 0 expense, 0 balance, d.updator entryby FROM donation d, scheme s, individual i, cost_center cc, cost_category ccat where d.donated_by_id=i.id and d.scheme_id=s.id and s.cc_id=cc.id and cc.cost_category_id="+ccat.id+" and d.fund_receipt_date >= '"+String.format('%tF',fd)+"'  and d.fund_receipt_date <= '"+String.format('%tF',td)+"' union SELECT c.name ccname,v.voucher_date date,v.voucher_no ref, v.description details, '','',v.amount_settled income, v.amount expense, 0 balance, v.updator entryby FROM voucher v, cost_center c, cost_category ccat where v.department_code_id=c.id and c.cost_category_id=ccat.id and ccat.id="+ccat.id+" and v.voucher_date >= '"+String.format('%tF',fd)+"' and v.voucher_date <= '"+String.format('%tF',td)+"') q  order by date"
	log.debug(query)
	def result = sql.rows(query)
	sql.close() 
	return result
