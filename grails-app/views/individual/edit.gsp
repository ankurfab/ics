...
<html>
    <head>
        ...
        <title>${individualInstance}(Edit)</title>
    </head>
                        ...
                        <tbody bgcolor="lavender">
                            ...
                            <tr>
                                ...
                                <td valign="top" class="value ${hasErrors(bean: individualInstance, field: 'legalName', 'errors')}">
                                    <g:textField name="legalName" maxlength="127" value="${individualInstance?.legalName}" size="50"/>
                                </td>
                                ...
                                <td valign="top" class="value ${hasErrors(bean: individualInstance, field: 'initiatedName', 'errors')}">
                                    <g:textField name="initiatedName" maxlength="127" value="${individualInstance?.initiatedName}" size="50"/>
                                </td>
                                <td rowspan="4">
...
<g:link controller="address" action="create" params="['individual.id': individualInstance?.id]" id="addAddress" target="_blank">${message(code: 'default.add.label', args: [message(code: 'address.label', default: 'Address')])}</g:link>
			<g:if test="${!(individualInstance?.address?.size()>0)}">
				<!--<g:link controller="address" action="create" params="['individual.id': individualInstance?.id]" id="addAddress" target="_blank">${message(code: 'default.add.label', args: [message(code: 'address.label', default: 'Address')])}</g:link>-->
				<g:textField name="addr" maxlength="127" value="" size="127"/>
			</g:if>
                            ...
                            <tr>
                                <td valign="top" class="name" width="12%">
                                  <label for="voiceContact"><b>Contact</b></label>
                                </td>
...
<g:link controller="voiceContact" action="create" params="['individual.id': individualInstance?.id]" target="_blank">Add Contact</g:link>
			<g:if test="${!(individualInstance?.voiceContact?.size()>0)}">
				<!--<g:link controller="voiceContact" action="create" params="['individual.id': individualInstance?.id]" target="_blank">Add Contact</g:link>-->
				<g:textField name="phone" maxlength="50" value="" size="50"/>
			</g:if>
...
<g:link controller="emailContact" action="create" params="['individual.id': individualInstance?.id]" target="_blank">${message(code: 'default.add.label', args: [message(code: 'emailContact.label', default: 'EmailContact')])}</g:link>
			<g:if test="${!(individualInstance?.emailContact?.size()>0)}">
				<!--<g:link controller="emailContact" action="create" params="['individual.id': individualInstance?.id]" target="_blank">${message(code: 'default.add.label', args: [message(code: 'emailContact.label', default: 'EmailContact')])}</g:link>-->
				<g:textField name="eml" maxlength="127" value="" size="127"/>
			</g:if>
...
<g:each in="${individualInstance?.relative1?}" var="r">
	...
	<g:if test="${isCouncelle == 'false'}">
		<br>
		<g:link controller="relationship" action="create" params="['individual1.id': individualInstance?.id, 'relationName' : 'Councellee of']" target="_blank">Add Councellor</g:link>
	</g:if>
		...
		<table border="0" cellspacing="0" cellpadding="0">
			<tbody bgcolor="lavender">
				<tr>
					...
					<td valign="top">
					   <!-- todo remove hardcoding for voice -->
					   <g:select size="30" name="roles" from="${ics.Role.findAllByCategory('Voice',[sort:'name'])}" optionKey="id"  value="${roleids}" noSelection="['':'-Choose roles-']" multiple="true"/>
					</td>                                
				</tr>
			</tbody>
		</table>
		...
		<table border="0" cellspacing="0" cellpadding="0">
			<tbody bgcolor="lavender">
				<tr>
					...
					<td valign="top">
					    <g:select size="30" name="servicesrendered" from="${ics.Seva.findAll("from Seva as s order by s.type, s.name")}" value="${sevarids}" optionKey="id"  noSelection="['':'-Choose Services Rendered-']" multiple="true"/>
					</td>                                
					...
					<td valign="top">
					    <g:select size="30" name="servicesinterested" from="${ics.Seva.findAll("from Seva as s order by s.type, s.name")}" value="${sevaiids}" optionKey="id"  noSelection="['':'-Choose Services Interested In-']" multiple="true"/>
					</td>                                
				</tr>
			</tbody>
		</table>
			...
			<tbody bgcolor="lavender">
				<tr>
					...
					<td valign="top">
					    ...
					    <g:select size="30" name="courses" from="${ics.Course.findAllByCategory('Voice',[sort:'name'])}" optionKey="id"  value="${courseids}" noSelection="['':'-Choose courses/workshops-']" multiple="true"/>
            				</sec:ifAllGranted>
            				<sec:ifAllGranted roles="ROLE_COUNSELLOR_GROUP">
					    <g:select size="30" name="courses" from="${ics.Course.findAllByCategory('Temple',[sort:'name'])}" optionKey="id"  value="${courseids}" noSelection="['':'-Choose Programs-']" multiple="true"/>
            				</sec:ifAllGranted>
            				<sec:ifNotGranted roles="ROLE_VOICE_ADMIN,ROLE_COUNSELLOR_GROUP">
					    <g:select size="30" name="courses" from="${ics.Course.list([sort:'name'])}" optionKey="id"  value="${courseids}" noSelection="['':'-Choose courses/workshops-']" multiple="true"/>
            				</sec:ifNotGranted>
					</td>                                
					...
					<td valign="top">
					    <g:select size="30" name="books" from="${ics.Book.findAllByAuthor('Srila Prabhupada',[sort:'name'])}" optionKey="id"  value="${bookids}" noSelection="['':'-Choose books read-']" multiple="true"/>
					</td>                                
				</tr>
			</tbody>
		</table>
	...
	<div class="dialog" id="SpiritualInformation">
		<table border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td valign="top">
					<g:select name="currentCentre" from="${ics.Centre.list(sort:'name')}" optionKey="id" value="${ics.IndividualCentre.findByIndividualAndStatus(individualInstance,'VALID')?.centre?.id}" noSelection="['':'-Choose-']"/>                                  
				</td>						
				<td valign="top">
			...
			<tbody bgcolor="lavender">
				<tr>
					...
					<td valign="top" class="value ${hasErrors(bean: individualInstance, field: 'title', 'errors')}" width="11%">
						<g:select name="title.id" from="${ics.Title.list()}" optionKey="id" value="${individualInstance?.title?.id}" noSelection="['null': '']" />
					</td>
				...
				<tr>
					...
					<td valign="top" class="value ${hasErrors(bean: individualInstance, field: 'isDonor', 'errors')}" width="11%">
						<g:checkBox name="isDonor" value="${individualInstance?.isDonor}" />
					</td>
					<!--<td valign="top" class="name" width="10%">
					...
					<td valign="top" class="value ${hasErrors(bean: individualInstance, field: 'isWellWisher', 'errors')}" width="11%">
						<g:checkBox name="isWellWisher" value="${individualInstance?.isWellWisher}" />
					</td>-->
            ...
            <sec:ifAnyGranted roles="ROLE_PATRONCARE,ROLE_PATRONCARE_USER">
				...
				<tr>
					...
					<td valign="top" class="value ${hasErrors(bean: individualInstance, field: 'status', 'errors')}" width="11%">
						<g:select name="status" from="${['ACTIVE','INACTIVE','VALID','INVALID']}" value="${individualInstance?.status}"  noSelection="['':'-Choose Status-']"/>
						<!--<g:textField name="status" value="${individualInstance?.status}" />-->
					</td>
				...
				<tr>
					...
					<td valign="top" class="value ${hasErrors(bean: individualInstance, field: 'businessRemarks', 'errors')}" width="11%" rowspan="5">
						<g:textArea name="businessRemarks" value="${individualInstance?.businessRemarks}" rows="5" cols="40"/>
					</td>
				...
				<tr>
					...
					<td valign="top" class="value ${hasErrors(bean: individualInstance, field: 'picFileURL', 'errors')}" width="11%">
						<g:textField name="picFileURL" value="${individualInstance?.picFileURL}" />
					</td>
					...
					<td valign="top" class="value ${hasErrors(bean: individualInstance, field: 'isLifeMember', 'errors')}" width="11%">                                    
						<g:checkBox name="isLifeMember" value="${individualInstance?.isLifeMember}" onClick="checkLifeMember() "  />
			...
			<tbody bgcolor="lavender">
				<tr >
					...
					<td valign="top" class="value ${hasErrors(bean: individualInstance, field: 'membershipNo', 'errors')}" width="11%">
						<g:textField name="membershipNo" value="${individualInstance?.membershipNo}" />
					</td>
					...
					<td valign="top" class="value ${hasErrors(bean: individualInstance, field: 'membershipPlace', 'errors')}" width="11%">
						<g:textField name="membershipPlace" value="${individualInstance?.membershipPlace}" />
					</td>
					<td>&nbsp;
					</td>
					<td>&nbsp;
		...
		<div id="Commitment">
			<table border="0" cellspacing="0" cellpadding="0">
				...
				<tr>
					...
					<td width="80%"><g:textField name="committedAmount" value="${ics.Commitment.findByCommittedBy(individualInstance)?.committedAmount}"/></td>
				</tr>
				<tr>
					<td width="20%"><b>Committed On</b></td>
					<td width="80%"><g:textField name="commitmentOn" value="${ics.Commitment.findByCommittedBy(individualInstance)?.commitmentOn?.format('dd-MM-yyyy')}"/></td>
				</tr>
				<tr>
					<td width="20%"><b>Committed Till</b></td>
					<td width="80%"><g:textField name="commitmentTill" value="${ics.Commitment.findByCommittedBy(individualInstance)?.commitmentTill?.format('dd-MM-yyyy')}"/></td>
				</tr>
