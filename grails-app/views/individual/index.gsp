...
<html>
	...
	<body>
		...
		<div id="dialogDonation" title="Donation">
			<form>
			<fieldset>
				<label for="From"><b>Range</b></nlabel><br><br>
				<!--<input type="text" id="Range" style="border:0; color:#f6931f; font-weight:bold;" size="30"/>
				<br>
				<div id="slider-range-donation"></div>-->
				<label for="minAmt">Minimum Amount</nlabel><br>
				<g:textField name="minAmt" value=""/><br><br>
				<label for="maxAmt">Maximum Amount</nlabel><br>
				<g:textField name="maxAmt" value=""/>
		...
		$( "#RangeMA" ).slider( "option", "animate", true);
		    ...
		    jQuery("#individual_list").jqGrid({
		      url:'jq_individual_list',
		      editurl:'jq_edit_individual',
		      datatype: "json",
		      colNames:['LegalName','InitiatedName','DoB','MarriageAnniv','DonationAmount','Address','Phone','Email','Cultivator','id'],
		      colModel:[
			{name:'legal_name', editable:false,
			formatter:'showlink', 
             		formatoptions:{baseLinkUrl:'show', addParam: '&category=edit'}
			},
			{name:'initiated_name', editable:false},
			{name:'dob', editable:false, search:false},
			{name:'marriage_anniversary', editable:false, search:false},
			{name:'donation_amount', editable:false, search:false},
			{name:'address'},
			{name:'phone', search:false},
			{name:'email', search:false},
		...
		jQuery('.ui-icon-refresh').click(function(){
		  jQuery("#individual_list").jqGrid('setGridParam',{url:"jq_individual_list?dobFlag=&maFlag=&donationFlag="}).trigger("reloadGrid");
		});
		
	// add custom button to export the detail data to excel	
	jQuery("#individual_list").jqGrid('navGrid',"#individual_list_pager").jqGrid('navButtonAdd',"#individual_list_pager",{caption:"Export", buttonicon:"ui-icon-disk",title:"Export",
	       onClickButton : function () { 
			jQuery("#individual_list").jqGrid('excelExport',{"url":'jq_individual_list'});
	       }
	       });
		...
		$( "#dialogDonation" ).dialog({
			...
			buttons: {
				"Submit": function() {
					...
					jQuery("#individual_list").jqGrid('setGridParam',{url:"jq_individual_list?donationFlag=1&minAmt="+$( "#minAmt" ).val()+"&maxAmt="+$( "#maxAmt" ).val()}).trigger("reloadGrid");
				},
				Cancel: function() {
					$( this ).dialog( "close" );
				}
