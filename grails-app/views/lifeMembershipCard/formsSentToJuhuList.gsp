...
<html>
	<head>
		...
		<title>Forms Sent To Juhu List</title>
	...
	<body>
		...
		<div id="formsSubmittedList-lifeMembershipCard" class="content scaffold-formsSubmittedList" role="main">
			...
			<table id="example">
				...
				<tbody>
				<g:each in="${formsSentToJuhuList}" status="i" var="lifeMembershipCardInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<sec:ifNotGranted roles="ROLE_PATRONCARE,ROLE_PATRONCARE_USER">
							<td><g:checkBox name="cardArrived" value="${lifeMembershipCardInstance?.id}" checked="false" /></td>
						</sec:ifNotGranted>
					</tr>
				</g:each>
				</tbody>
			...
			<div class="pagination">
				<g:paginate total="${formsSentToJuhuListTotal}" />
			</div>
			<sec:ifNotGranted roles="ROLE_PATRONCARE,ROLE_PATRONCARE_USER">
			    <br>
			    <div class="buttons">
				<g:form name="markAsCardArrivedForm" controller="lifeMembershipCard" action="markAsCardArrived" >
					<g:hiddenField name="lifePatronCardIds" id="lifePatronCardIds" value=""/>
					<span class="button"><g:actionSubmit name="markAsCardArrived" class="save" action="markAsCardArrived" onclick="markAsCardArrived()" value="Mark as 'Card Arrived'" /></span>
				</g:form>
			    </div>
			</sec:ifNotGranted>
		</div>
		<script language="javascript"> 		
			...
			{
				var formSentToJuhuChkBoxes = document.getElementsByName("cardArrived");
		...
		<g:javascript>
			function markAsCardArrived()
			{
				var CardArrivedChkBoxes = document.getElementsByName("cardArrived");
				var lifePatronCardIds = new Array();

				for(i=0; i< CardArrivedChkBoxes.length; i++)
				{
					if(CardArrivedChkBoxes[i].checked == true)
						lifePatronCardIds[i] = CardArrivedChkBoxes[i].value;
				}
				if(lifePatronCardIds.length == 0)
				{
					alert("Please select records(s) to be marked as 'Card Arrived");
					return false;
				}
				else
				{
